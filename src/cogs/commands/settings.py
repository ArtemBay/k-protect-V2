...
class Settings(commands.Cog):
    ...
    async def muterole(self, ctx, role: discord.Role = None):
        ...
        if role is None:
            try:
                ...
                mr = 'Не указана'
            embed = discord.Embed(color=Color.succes)
        ...
        if messages.is_admin(ctx.author):
            ...
            if punishment.lower() in list(punishments) or punishment.lower() == 'show':
                ...
                if punishment.lower() == 'show':
                    if ti > 0:
                        await ctx.send(f'<:__:1012347971850993706> Наказание для пригласившего краш-бота: **{punishments[p]}** на **{word.hms2(ti)}**.')
                    else:
                        await ctx.send(f'<:__:1012347971850993706> Наказание для пригласившего краш-бота: **{punishments[p]}**.')
                elif punishment.lower() == 'kick':
                    if word.string_to_seconds(t) == 0:
                        n['nuker-type'] = 'kick'
                        await ctx.send('<a:green_check:1012069652173697156> Теперь пригласившего краш-бота ждёт **кик**.')
                    else:
                        await ctx.send('<:KPcancel:1016403449996398592> Кик не имеет настройки по времени.')
                    ...
                    if word.string_to_seconds(t) > 0:
                        ...
                        blob = ''
                    await ctx.send(f'<a:green_check:1012069652173697156> Теперь пригласившего краш-бота ждёт **бан**{blob}.')
                elif punishment.lower() == 'none':
                    if word.string_to_seconds(t) == 0:
                        n['nuker-type'] = 'none'
                        await ctx.send('<a:green_check:1012069652173697156> Теперь пригласившего краш-бота ничего не ждёт.')
                    else:
                        await ctx.send('<:KPcancel:1016403449996398592> Отсутствие наказания не имеет настройки по времени.')
            else:
                await ctx.send('<:KPcancel:1016403449996398592> Неизвестная опция.')
        ...
        if messages.is_admin(ctx.author):
            if not ctx.guild.id in cache.configs_data:
                ...
                if not 'roleprotect' in data:
                    ...
                    rp = data['roleprotect']
            if option is None:
                embed = discord.Embed(title = "<:KPprotect:1016401995151720499> | Защита ролей")
                if rp == 0:
    ...
    async def warn_actions(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            p = ctx.prefix
            embed = discord.Embed(color=Color.success)
            embed.title = "<:__:1012347971850993706> | Наказания за предупреждения"
            embed.description = f'''
...
`{p}warn_actions reset` – Сбросить все наказания
    ...
    async def antiraid(self, ctx, joins: int = None, interval = None):
        ...
        if joins is not None and interval is not None:
            ...
            if joins < 0:
                ...
                embed = discord.Embed(color = Color.success, title = "<a:green_check:1012069652173697156> | Готово")
                if turn == 0:
            ...
            embed = discord.Embed(title = "<:KPprotect:1016401995151720499> | Защита от рейдов", color = Color.success)
            if turn == 0:
    ...
    async def show(self, ctx, cmd):
        ...
        embed = discord.Embed(color=Color.success)
        if cmd != "*": embed.title = f"<:KPsupport:1016365379351740528> | Права для команды `{ctx.prefix}{cmd}`"
        else: embed.title = "<:KPsupport:1016365379351740528> | Права для всех команд"
    ...
    async def notify_dm(self, ctx, option=None):
        ...
        embed = discord.Embed(title="<:Announcement_green:1012070027756851363> | Оповещения о наказаниях в личку", color=Color.success)
        if not option:
    ...
    async def score(self, ctx, option=None, amount: int=None):
        if not option:
            ...
            embed = discord.Embed(title="<:Member_green:1012069666572746752> | Баллы за краш")
...
**Максимум:** {max}
            ...
            embed = discord.Embed(title="<a:green:1012043532120367124> | Справка по фильтрам")
            embed.description = """
